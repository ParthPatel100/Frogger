...
bool isGameWon(Pixel *pixel)
{
	...
	if (frog.lane == 1)
	{
		for (int i = 0; i <= 4; i++)
		{
			...
			{
				cleanBackground(pixel, frog.position, 187, frog.position + 33, 187 + 33);
				if (i == 0)
				{
...
{
	//~ short int *gameMenuPtr = (short int *) gameMenu.pixel_data;
	//~ int i = 0;
	//~ for (int y = 245; y < 545; y++)
	//~ {
		//~ for (int x = 202; x < 602; x++) 
		//~ {	 
			...
			//~ pixel->y = y;
			//~ drawPixel(pixel);
			//~ game.pausescreen[800 * y + x] = gameMenuPtr[i];
		//~ }
